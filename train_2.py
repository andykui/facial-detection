...
EPOCHS = 100             #迭代次数
BATCH_SIZE = 107        #每个batch大小，稍微大一点的batch会更稳定
...
def model():
   ...
   W_fc1 = weight_variable([5 * 5 * 256, 500])
...
if __name__ == '__main__':
    ...
    for i in xrange(EPOCHS):
        ...
        for j in xrange(0,TRAIN_SIZE,BATCH_SIZE):
            ...
            train_step.run(feed_dict={x:X_train[j:j+BATCH_SIZE], 
                y_:y_train[j:j+BATCH_SIZE], keep_prob:0.6})
            train_accuracy = rmse.eval(feed_dict={x:X_train[j:j+BATCH_SIZE], y_:y_train[j:j+BATCH_SIZE], keep_prob: 1.0})
            print("step %d, training accuracy %g"%(i, train_accuracy))

        print("step %d, training accuracy %g"%(i, train_accuracy))
        if train_accuracy < 0.06 : 
            break;
        #for j in xrange(0,TRAIN_SIZE,BATCH_SIZE):
        #    print 'epoch {0}, train {1} samples done...'.format(i,j)

        #    train_accuracy = accuracy.eval(feed_dict={x:X_train[j:j+BATCH_SIZE], 
        #        y_:y_train[j:j+BATCH_SIZE], keep_prob: 1.0})
        #    print("step %d, training accuracy %g"%(i, train_accuracy))
        ...
        #print 'epoch {0} done! validation loss:{1}'.format(i, train_accuracy*96.0)
       # if validation_loss > train_accuracy:
    ...
    save_path = saver.save(sess, "model/model-soft.ckpt")
    print 'save model'
